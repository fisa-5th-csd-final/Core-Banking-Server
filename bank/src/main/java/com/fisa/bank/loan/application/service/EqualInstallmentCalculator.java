...
public class EqualInstallmentCalculator {
    ...
    {
        // 월 금리 계산
        BigDecimal monthlyRate = completedInterest.divide(BigDecimal.valueOf(12), 10, RoundingMode.HALF_DOWN);

        // (r * P) / (1 - (1 + r)^-n)
        BigDecimal numerator = monthlyRate.multiply(principal); //r * P
        BigDecimal denominator = BigDecimal.ONE.subtract(
                BigDecimal.ONE.add(monthlyRate).pow(-term, new java.math.MathContext(10, RoundingMode.HALF_DOWN))
        );

        BigDecimal monthlyPayment = numerator.divide(denominator, 0, RoundingMode.DOWN);
        BigDecimal interestPayment = remainPrincipal.multiply(monthlyRate).setScale(0, RoundingMode.DOWN);
